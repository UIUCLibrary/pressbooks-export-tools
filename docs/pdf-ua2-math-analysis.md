...
With `tagging=on`, luamml automatically:
...
- Works automatically once `\DocumentMetadata{tagging=on}` is added

The only question is whether luamml-generated MathML is acceptable, or whether MathJax MathML must be preserved. Start with luamml (simpler), fall back to external injection if needed.
